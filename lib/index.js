...
var undoable = function undoable(reducer) {
  ...
  if (options.include.length > 0 && options.exclude.length > 0) {
    console.error("".concat(DEBUGPREPEND, " cannot have both a 'include' and 'exclude' property, choose one or the other!"));
    return;
  }
      ...
      default:
        {
          ...
          if (options.include.length > 0 && !options.include.includes(action.type) || options.exclude.length > 0 && options.exclude.includes(action.type)) {
            return {
              past: past,
              present: newPresent,
              future: []
